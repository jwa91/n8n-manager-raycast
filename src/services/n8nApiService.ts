...
import type { AxiosError, Method as AxiosMethod, AxiosRequestConfig } from "axios"
import type { N8nApiPathParams, Preferences, WorkflowFull, WorkflowSummary } from "../types"
...
async function validateN8nPreferences(): Promise<Pick<Preferences, "n8nWebhookUrl" | "apiToken">> {
...
function buildN8nQueryParams(params?: N8nApiPathParams): Record<string, string> {
...
function extractN8nErrorMessage(err: AxiosError): string {
  ...
  if (err.message && !err.message.toLowerCase().includes("request failed with status code")) {
    return err.message
  }
...
async function baseN8nApiRequest<T = unknown>(
  ...
  const config = buildN8nAxiosConfig(n8nWebhookUrl, method, headers, queryParams, requestBody)
...
export async function getWorkflowSummaries(
  ...
  const params: N8nApiPathParams = { mode: "summary", includedWorkflows: filter }
  const response = await baseN8nApiRequest<WorkflowSummary[] | WorkflowSummary>("GET", params)
...
function normalizeAndValidateWorkflowDetails(
  ...
  } else if (typeof responseData === "object" && responseData !== null && "id" in responseData) {
...
export async function getWorkflowDetails(workflowId: string): Promise<WorkflowFull | null> {
  ...
  const response = await baseN8nApiRequest<WorkflowFull | WorkflowFull[]>("GET", params)
